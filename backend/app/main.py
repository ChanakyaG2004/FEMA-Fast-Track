...
async def analyze_claim(request: Request) -> AnalyzeClaimResponse:
    ...
    if refusal:
        return AnalyzeClaimResponse(status=ClaimStatus.NEEDS_INFO, missing_fields=missing, question=None, claim=state.claim, session_state=state, refusal=refusal, legal_citations=citations, evidence_items=state.evidence_items, evidence_warnings=evidence_warnings)

    if missing:
        return AnalyzeClaimResponse(status=ClaimStatus.NEEDS_INFO, missing_fields=missing, question=next_question(missing), claim=state.claim, session_state=state, legal_citations=citations, evidence_items=state.evidence_items, evidence_warnings=evidence_warnings)
    ...
    return AnalyzeClaimResponse(status=ClaimStatus.COMPLETE, missing_fields=[], question=None, claim=state.claim, session_state=state, pdf_base64=pdf_base64, pdf_url=pdf_data_url(pdf_base64), filename="fema-fast-track-claim.pdf", legal_citations=citations, evidence_items=state.evidence_items, evidence_warnings=evidence_warnings, red_team_notes=red_team_notes)
